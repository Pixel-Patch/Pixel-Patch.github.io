...
<body class="sidebar-noneoverflow">
    ...
    $userIdQuery = "SELECT `V_USERID`, `image`, `imageDL`, `userType`, `Rank`, `Branch_of_Service`, `Fullname`, `Cellphone_Number`, `Date_of_Birth`, `Address`, `Office`, `Gender`, `Vehicle_Model`, `Vehicle_Type`, `Vehicle_Color`, `Vehicle_Plate_Number` FROM `v_user` WHERE ID = $id";
    ...
    if ($userIdResult && $userIdResult->num_rows > 0) {
        ...
        if (isset($_POST['submit'])) {
            ...
            $newData = array(
                'userType' => $userType,
                'Rank' => $rank,
                'Branch_of_Service' => $bos,
                'Fullname' => $fullname,
                'Cellphone_Number' => $cellno,
                'Date_of_Birth' => $dob,
                'Address' => $address,
                'Office' => $office,
                'Gender' => $gender,
                ...
                'Vehicle_Plate_Number' => $v_plate
            );

            // Find the changed columns and their corresponding new values
