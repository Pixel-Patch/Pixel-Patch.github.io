					...
					<div class="row layout-top-spacing">
						<div class="col-12 layout-spacing">
							...
							<div class="widget widget-chart-three">
								<div class="widget-heading">
									<div class="">
										<h5 class="">Time In / Time Out Log</h5>
									</div>
						...
						<div class="col-xl-6 col-lg-6 col-md-6 col-sm-12 col-12 layout-spacing">
							<div class="widget-four">
								<div class="widget-heading">
									<h5 class="">Time In / Time Out Log Percentage</h5>
								</div>
								<div class="widget-content">
								...
								<div class="widget-content">
									<div class="mt-container mx-auto">
										<div class="timeline-line">
											...
											<?php
											$query = "SELECT `id`, `name`, `action`, `user_id`, `table_name`, `table_column`, `old_value`, `new_value`, `timestamp` FROM `activity_log` WHERE 1 ORDER BY `timestamp` DESC";

											$result = mysqli_query($conn, $query); // Assuming $conn is your database connection

											while ($row = mysqli_fetch_assoc($result)) {
												// Determine the color class based on the action
												$class = '';
												switch ($row['action']) {
													case 'Added':
														$class = 'primary';
												...
												// Display the activity log entry
												echo '<div class="item-timeline timeline-' . $class . '">';
												echo '    <div class="t-dot" data-original-title="" title=""></div>';
												echo '    <div class="t-text">';
												echo "        <p><span>{$row['name']}</span> {$row['action']} <span>{$row['user_id']}</span></p>";
												echo "        <span class=\"badge badge-{$class}\">{$row['action']}</span>";
												echo "        <p class=\"t-time\">{$row['timestamp']}</p>";
												echo '    </div>';
												echo '</div>';
											}
